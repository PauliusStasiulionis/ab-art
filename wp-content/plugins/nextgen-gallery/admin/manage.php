...
class nggManageGallery {
	...
	function update_pictures() {
		...
		$taglist = 		isset ( $_POST['tags'] ) ? $_POST['tags'] : false;
                $long_description = isset ( $_POST['long_description']) ? $_POST['long_description'] : array();
		...
		if ( is_array($pictures) ){
			foreach( $pictures as $pid ){
                ...
                if ($image) {
                    ...
                    $image->description_long = $long_description[$image->pid];
                    ...
                    // update the database
                    $wpdb->query( $wpdb->prepare ("UPDATE $wpdb->nggpictures SET image_slug = '%s', alttext = '%s', description = '%s',description_long= '%s', exclude = %d WHERE pid = %d", 
                                                                                 $image->image_slug, $image->alttext, $image->description,$image->description_long , $image->exclude, $image->pid) );    
                    // remove from cache    
