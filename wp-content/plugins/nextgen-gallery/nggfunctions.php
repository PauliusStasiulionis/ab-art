...
function nggShowGallery( $galleryID, $template = '', $images = false ) {
    ...
    if ( !is_home() || $pageid == get_the_ID() ) { 
        ...
        if ($show == 'about'){
            $out = '<div class="ngg-aboutpic">';
            $out .= nggShowAbout($pid);
            $out .= '</div>'."\n";
            return $out;
        }
        if  ($show == 'picform'){
            
            $out = '<div class="ngg-form">';
            $out .= nggShowPicForm($pid);
            $out .= '</div>'."\n";
            return $out;
        }
    }
...
function nggShowPicForm($imageID, $template = ''){
    
    $picture = nggdb::find_image($imageID);
    
    $filename = ( empty($template) ) ? 'picform' : 'picform-' . $template;
    $meta = new nggMeta($imageID);
    
    //$meta->sanitaze();
    $db = $meta->get_saved_meta();
    $_POST['picid'] = $imageID;
    $_POST['picname'] = $picture->alttext;
    $_POST['picfile'] = $picture->filename;
    $out = nggGallery::capture ( $filename, array ('image' => $picture , 'db' => $db) );
    return $out;
    
}
function nggShowAbout($imageID, $template = ''){
    
    global $nggRewrite;
    $picture = nggdb::find_image($imageID);
    $filename = ( empty($template) ) ? 'about' : 'about-' . $template;
    $meta = new nggMeta($imageID);
    $meta->sanitize();
    $db = $meta->get_saved_meta();

    
    $picture->picture_form_link =  $nggRewrite->get_permalink(array ( 'show' => 'picform', 'pid' => $imageID) );
...
function nggCreateGallery($picturelist, $galleryID = false, $template = '', $images = false) {
    ...
    foreach ($picturelist as $key => $picture) {
        ...
        $picturelist[$key]->about_link =  $nggRewrite->get_permalink(array ( 'pageid' => $picture->pageid, 'show' => 'about', 'pid' => $picture->pid ) );
