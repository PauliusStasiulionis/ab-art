...
class WP_Widget_Page extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_page', 'description' => __( 'Your page, WordPress Pages') );
		parent::__construct('page', __('Page'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		$exclude = empty( $instance['exclude'] ) ? '' : $instance['exclude'];
                $page_data = get_page($exclude);
                
                $title = empty( $instance['title'] ) ? '<a href="'.  get_permalink($exclude).'" title="'.$page_data->post_title.'">'.$page_data->post_title.'</a><h3>' : '<h3 class="widget-title">'.$instance['title'];
               
                $out = $page_data->post_content;
                $out = preg_replace('/<img[^>]+./' , '' , $out);
                if(strlen($out) > 200){
                    $out = substr($out, 0, 200). ' <a href="'.  get_permalink($exclude).'" title="'.$page_data->post_title.'">...</a>';
                }
