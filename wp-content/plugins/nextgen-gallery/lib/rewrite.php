	...
	function nggRewrite() {
        ...
        $this->ngg_rules = array(
    		...
    		'/slideshow/' => '&show=slide',
                '/about/([^/]+)/' => '&show=about&pid=[matches]',
                '/picform/([^/]+)/' => '&show=picform&pid=[matches]',
    		'/images/' => '&show=gallery',
    		...
    		'/([^/]+)/([^/]+)/slideshow/' => '&album=[matches]&gallery=[matches]&show=slide',
                '/([^/]+)/([^/]+)/about/([^/]+)/' => '&album=[matches]&gallery=[matches]&show=about&pid=[matches]',
                '/([^/]+)/([^/]+)/picform/([^/]+)/' => '&album=[matches]&gallery=[matches]&show=picform&pid=[matches]',
    		...
    		'/([^/]+)/([^/]+)/page-([0-9]+)/slideshow/' => '&album=[matches]&gallery=[matches]&nggpage=[matches]&show=slide',
                '/([^/]+)/([^/]+)/page-([0-9]+)/about/([^/]+)/' => '&album=[matches]&gallery=[matches]&nggpage=[matches]&show=about&pid=[matches]',
                '/([^/]+)/([^/]+)/page-([0-9]+)/picform/([^/]+)/' => '&album=[matches]&gallery=[matches]&nggpage=[matches]&show=picform&pid=[matches]',
		...
		if ($wp_rewrite->using_permalinks() && $this->options['usePermalinks'] ) {
			...
			**/
            ...
            if(isset($args['pageid']))
                $url = trailingslashit ( get_permalink ($args['pageid']) ) . $this->slug;  
            else
            ...
            if (($show_on_front == 'page') && ($page_on_front == get_the_ID()) && !isset($args['pageid'])){
			...
			if  (isset ($args['pid']) && !isset ($args['show']))
				$url .= '/image/' . $args['pid'];			
			...
			if  (isset ($args['show']) )
                        ...
                        if  (isset ($args['show']) && isset($args['pid'])){
                                if($args['show'] == 'about')
                                    
                                    $url .= '/about/' . $args['pid']. "/";
                                if($args['show'] == 'picform')
                                    $url .= '/picform/' . $args['pid']."/";
                        }
	...
	function rewrite_title($title) {
		...
		elseif ( $show == 'show' )
			$new_title .= __('Gallery', 'nggallery') . $sep ;	
                elseif ( $show == 'about') 
                        $new_title .= __('About', 'nggallery') . $sep ;
		if ( !empty($pid) )
